...
def _validate_shop_hours_and_past(day: date, start_t: time, end_t: time) -> str | None:
    ...
    start_dt = datetime.combine(day, start_t, SHOP_TIMEZONE)

    # Prefer a clear "past" message if the user picked any time in the past
    if start_dt < now:
    ...
    if end_t > LAST_END_TIME:
        return "End time must be 18:30 or earlier."
...
def cancel_booking(booking_id: int):
    ...
    # Phone must match booking phone
    booking_phone = _normalize_phone(booking.get("customer_phone"))
    if not booking_phone or booking_phone != phone:
        return (
            jsonify(
                {"ok": False, "error": "Phone number does not match this booking."}
            ),
            403,
        )
